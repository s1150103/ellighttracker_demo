...
class ElWorkRecord:
    ...
    def save(self):
        ...
        entity = datastore.Entity(key=key)
        # ✅ uptime を自動計算（end_time があるときのみ）
        uptime = None
        if self.end_time:
            uptime = (self.end_time - self.start_time) // 1000  # 秒単位で計算
        entity.update({
            'regionId': self.region_id,
            'targetId': self.target_id,
    ...
    def update_end_time(self, end_time):
        ...
        if entity:
            ...
            uptime = (end_time - self.start_time) // 1000  # 秒に変換
            entity['uptime'] = uptime
            datastore_client.put(entity)
            print(f"✅ ElWorkRecord 更新: {self.id} (終了: {end_time}, uptime: {uptime} 秒)")
...
@app.route('/KFormPaper3.html')
def serve_kunugi_form():
    return send_from_directory(os.path.join(app.root_path, 'static'), 'KFormPaper3.html')

# -- 静的HTMLファイルの提供: KunugiPaper5.html を公開 --
@app.route('/KunugiPaper.html')
def serve_kunugi():
    return send_from_directory(os.path.join(app.root_path, 'static'), 'KunugiPaper.html')

# -- 静的HTMLファイルの提供: KunugiPaper5.html を公開 --
@app.route('/Kunugi_graph.html')
def serve_kunugi_graph():
    return send_from_directory(os.path.join(app.root_path, 'static'), 'Kunugi_graph.html')
...
def process_each_mode(year, month, region_id):
    ...
    for record in records:
        ...
        if start_time and end_time:
            uptime = (end_time - start_time) // 1000  # 秒単位
        ...
        response_json_list.append({
            ...
            "upTime": uptime if uptime is not None else 0,  # ← ここで計算値を反映
            "date": record.get('date', '')
        })
        response_body = {
            "datalist": response_json_list
        }
...
def process_total_mode(year, month, region_id):
    ...
    eq_map = {}  # targetId → targetName
    counter_map = defaultdict(lambda: {"count": 0, "seconds": 0})  # key = f"{targetId}@{date}"
    ...
    for rec in records:
        ...
        uptime = max(0, math.ceil((end_time - start_time) / 1000))  # 秒に切り上げ
        ...
        date = int(rec["date"])
        key = f"{target_id}@{date}"

        # 機器名キャッシュ
        if target_id not in eq_map:
            target_key = datastore_client.key('ElTarget', f"{region_id}#{target_id}")
            target_entity = datastore_client.get(target_key)
            if target_entity:
                eq_map[target_id] = target_entity.get("targetName", "名前なし")
            else:
                eq_map[target_id] = "UNKNOWN"

        counter_map[key]["count"] += 1
        counter_map[key]["seconds"] += uptime
    ...
    for day in range(1, days_in_month + 1):
        ...
        for tid in eq_map:
            key = f"{tid}@{day}"
            count = counter_map[key]["count"]
            seconds = counter_map[key]["seconds"]
            obj[f"cycle{tid}"] = count
            obj[f"time{tid}"] = format_seconds(seconds)
        dataObjectList.append(obj)

    # 機器一覧
    eqList = [{"id": tid, "name": eq_map[tid]} for tid in sorted(eq_map)]
    ...
    print("🌐 レスポンスデータ:", json.dumps(result, indent=2, ensure_ascii=False))
